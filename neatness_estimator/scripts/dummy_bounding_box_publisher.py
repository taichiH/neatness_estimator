...
from neatness_estimator_msgs.srv import UpdateBoundingBox, UpdateBoundingBoxResponse
...
class BoundingBoxArrayPublisher(object):
    ...
    def __init__(self):
        ...
        self.service_server= rospy.Service('~update_boxes', UpdateBoundingBox, self.service_callback)
    ...
    def service_callback(self, req):
        res = UpdateBoundingBoxResponse()
        try:
            boxes = []
            for box in req.boxes.boxes:
                pos = [box.pose.position.x, box.pose.position.y, box.pose.position.z]
                rot = list(euler_from_quaternion(
...
if __name__ == '__main__':
    rospy.init_node('dummy_bounding_box_array_publisher')
